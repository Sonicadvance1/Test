...
void sha256_process( context_sha256_t *ctx, char data[64] )
{
...
namespace SHA256
{
	...
	void Finish(u8 *Out)
	{
		...
		Update( (char*)sha256_padding, padn );
		Update( msglen, 8 );

		PUT_UINT32( ctx.state[0], Out,  0 );
		PUT_UINT32( ctx.state[1], Out,  4 );
		PUT_UINT32( ctx.state[2], Out,  8 );
		PUT_UINT32( ctx.state[3], Out, 12 );
		PUT_UINT32( ctx.state[4], Out, 16 );
		PUT_UINT32( ctx.state[5], Out, 20 );
		PUT_UINT32( ctx.state[6], Out, 24 );
		PUT_UINT32( ctx.state[7], Out, 28 );
	}
}
